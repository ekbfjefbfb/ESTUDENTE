...
async def perform_web_search(
    ...
    if tavily_sources:
        ...
        logger.info(
            f"web_search_provider user_id={user_id} provider=tavily "
            f"meta={_safe_meta(tavily_meta)} results={len(ddg_sources)} "
            f"duration_ms={int((time.monotonic()-ws_t0)*1000)}"
        )
        return ddg_sources, {"provider": "tavily", "meta": tavily_meta}
    ...
    if ddg_sources:
        logger.info(
            ...
            f"meta={_safe_meta(serper_meta)} results={len(ddg_sources)} "
            f"duration_ms={int((time.monotonic()-ws_t0)*1000)}"
        )
        return ddg_sources, {"provider": "serper", "meta": serper_meta}
    
    # 3) DDG fallback
    try:
        ddg_sources, ddg_meta = await ddg_search_service.search_with_meta(query.strip())
    except Exception:
        ddg_sources = []
        ddg_meta = {"status": "failed"}
    
    logger.info(
        f"web_search_provider user_id={user_id} provider=ddg "
        f"meta={_safe_meta(ddg_meta)} results={len(ddg_sources)} "
        f"duration_ms={int((time.monotonic()-ws_t0)*1000)}"
    )
    
    return ddg_sources, {"provider": "ddg", "meta": ddg_meta}
